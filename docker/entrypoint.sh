...
# `docker run` / docker-compose deployment would set directly (NETSCAN_*,
# see docker-compose.yml), then fall through to the normal server entrypoint
# either way. The add-on's port is fixed (netscan/config.yaml's ingress_port
# assumes 8099), so only default_subnet/interval are option-driven.
set -e
